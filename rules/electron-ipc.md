...
- For **non-bug** failures (validation, not found, auth, user refusal, etc.), prefer `MinervaError` with the right `MinervaErrorKind` so PostHog does not flood with `$exception` events — see [rules/dyad-errors.md](dyad-errors.md).
